...
age_part_spec_id = spec_id % 2
sd_part_spec_id = spec_id//2

sd_multipliers = np.array([.25,.5])
this_sdmult=sd_multipliers[sd_part_spec_id]

print(f"multiplier: {this_sdmult}")

## First
interactive = False
save = False
test_value = False  # wether to compute aesera test value
prefix = "cc-Fit-hist-review-lowbase"

dynamics_specs = ["mvDynamics"]  # "inDynamics",
age_spec = ["U5", "adult"]
model_specs = [f"{a}-{d}" for a in age_spec for d in dynamics_specs]
model_spec = model_specs[age_part_spec_id]
model_id = f"{prefix}_{model_spec}_{utils.get_git_revision_short_hash()}_{datetime.date.today()}_lowbase{this_sdmult}"
model_folder = f"/work/users/c/h/chadi/calib_Fit-hist-review-lowbase/{model_id}"
    ...
    with pm.Model() as titer_indivdecay_model4:
        ...
        if dynamics_spec == "inDynamics":
            ...
            titer_params = pm.MvNormal("titer_params", 
                    mu = [2.98825782+np.log2(5)-3.9655*this_sdmult, np.log2(45)], 
            ...
            omega_p =  pm.Deterministic("omega_p", ptt.switch(omega_p_t>0, omega_p_t, 0))
            llambda =  pm.Deterministic("lambda", ptt.switch(llambda_t>0, llambda_t, 0))
        else:
